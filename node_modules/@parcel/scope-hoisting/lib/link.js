      ...
      if (callee.name === '$parcel$require') {
        ...
        if (!bundleGraph.getDependencyResolution(dep, bundle)) {
          ...
          if (bundle.env.outputFormat !== 'commonjs') {
            ...
            throw new Error("`require.resolve` calls for excluded assets are only supported with outputFormat = 'commonjs'");
          }
          ...
          // TODO add loc information once available
          throw new Error("`require.resolve` calls for bundled modules or bundled assets aren't supported with scope hoisting");
        }
      }
    },
    ...
    Program: {
      exit(path) {
        ...
        for (let file of importedFiles.values()) {
          if (file.bundle) {
            format.generateBundleImports(bundle, file.bundle, file.assets, path);
          } else {
